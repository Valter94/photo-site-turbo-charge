...
-- Добавить UNIQUE constraint для предотвращения дублей в будущем
-- (На случай если будет только одна запись)
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'site_settings_unique' 
        AND table_name = 'site_settings'
    ) THEN
        -- Сначала убедимся что есть только одна запись
        DELETE FROM site_settings 
        WHERE id NOT IN (
        ...
        ADD CONSTRAINT site_settings_unique 
        CHECK ((SELECT COUNT(*) FROM site_settings) <= 1);
    END IF;
END $$;
