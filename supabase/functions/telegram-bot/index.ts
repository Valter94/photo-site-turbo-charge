        ...
        // Portfolio category selection
          ...
          if (session && session.type === 'portfolio' && session.step === 'choosing_category') {
            ...
            await telegramAPI.editMessage(
              ...
              messageId!,
              `📝 <b>Шаг 3: Описание</b>\n\n` +
              `✅ Категория: <b>${categoryMap[category]}</b>\n` +
      ...
      if (session) {
        try {
          if (session.step === 'waiting_title') {
            ...
            if (!fileData.ok) {
              throw new Error(`Не удалось получить файл: ${fileData.description}`)
            }
            ...
            if (session.type === 'portfolio') {
              ...
              await telegramAPI.sendMessage(
                ...
                `🏷 Категория: ${categoryNames[session.data.category]}\n` +
