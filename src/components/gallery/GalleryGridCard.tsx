...
interface GalleryGridCardProps {
  ...
  onView: (e: React.MouseEvent | undefined) => void;
  onClick: () => void;
  onMouseEnter: () => void;
  onMouseLeave: () => void;
}
...
const GalleryGridCard: React.FC<GalleryGridCardProps> = ({
  ...
  return (
    <Card
      ...
      } bg-white/90 backdrop-blur-sm border-0 shadow-lg hover:shadow-pink-200/50`}
      onClick={onClick}
    ...
    >
      <div className={`relative overflow-hidden rounded-t-lg ${
        ...
        <div
          className={`absolute inset-0 flex items-center justify-center transition-all duration-300 ${
            isHovered || isMobile ? "opacity-100 bg-black/40 backdrop-blur-sm" : "opacity-0"
          }`}
        >
      ...
      <div className="p-4 sm:p-6 bg-gradient-to-br from-white to-gray-50/50">
        <h3 className={`font-bold mb-3 text-gray-900 group-hover:text-pink-600 transition-colors line-clamp-2 ${
